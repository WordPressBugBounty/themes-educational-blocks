 ...
 */
if (!function_exists('educational_blocks_is_plugin_installed')) {
    function educational_blocks_is_plugin_installed($plugin_slug)
    {
        $plugin_path = WP_PLUGIN_DIR . '/' . $plugin_slug;
        return file_exists($plugin_path);
    }
}
if (!function_exists('educational_blocks_is_plugin_activated')) {
    function educational_blocks_is_plugin_activated($plugin_slug)
    {
...
add_action('wp_ajax_educational_blocks_install_and_activate_plugins', 'educational_blocks_install_and_activate_plugins');
add_action('wp_ajax_nopriv_educational_blocks_install_and_activate_plugins', 'educational_blocks_install_and_activate_plugins');
add_action('wp_ajax_educational_blocks_rplugin_activation', 'educational_blocks_rplugin_activation');
add_action('wp_ajax_nopriv_educational_blocks_rplugin_activation', 'educational_blocks_rplugin_activation');
...
function educational_blocks_all_plugins_active() {
...
function educational_blocks_install_and_activate_plugins() {
    if (!current_user_can('manage_options')) {
        return;
    }
    check_ajax_referer('educational_blocks_welcome_nonce', 'nonce');
    ...
    foreach ($recommended_plugins as $plugin) {
        ...
        if (is_educational_blocks_plugin_installed($plugin_slug)) {
    ...
    $redirect_url = admin_url('themes.php?page=educational-blocks-pro');
    echo json_encode([
        'success' => true,
        'data' => [
...
function is_educational_blocks_plugin_installed($plugin_slug) {
...
add_action('wp_ajax_educational_blocks_dismissed_notice_handler', 'educational_blocks_ajax_notice_dismiss_function');

function educational_blocks_ajax_notice_dismiss_function() {
    if (!wp_verify_nonce($_POST['wpnonce'], 'educational_blocks_welcome_nonce')) {
...
function educational_blocks_custom_admin_notice() {
    if (!get_option('dismissed-get_started_notice', false)) {
        ...
        if ($educational_blocks_current_screen && $educational_blocks_current_screen->id !== 'appearance_page_educational-blocks-pro') {
            ?>

            <div class="educational-blocks-admin-notice notice notice-info is-dismissible content-install-plugin theme-info-notice" id="educational-blocks-dismiss-notice" data-notice="get_started_notice">
                <div class="notice-div">
                    ...
                    <div class="notice-buttons-box">
                        ...
                        <button class="button-primary educational-blocks-notice-install-btn">
                            <span class="dashicons dashicons-download"></span>
                            <?php esc_html_e( 'Begin Installation', 'educational-blocks' ); ?>
                        </button>
                        <?php endif; ?>
                        <a class="button-primary getstarted" href="<?php echo esc_url( admin_url( 'themes.php?page=educational-blocks-pro' ) ); ?>" target="_blank"><?php esc_html_e('Theme Informations', 'educational-blocks'); ?></a>
