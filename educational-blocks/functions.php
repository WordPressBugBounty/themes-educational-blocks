...
function educational_blocks_scripts() {
    ...
    //homepage slider
	wp_enqueue_style('swiper-bundle-css', get_template_directory_uri() . '/assets/css/swiper-bundle.css', array(), EDUCATIONAL_BLOCKS_VERSION);

	wp_enqueue_script('swiper-bundle-js', get_template_directory_uri() . '/assets/js/swiper-bundle.js', array('jquery'), EDUCATIONAL_BLOCKS_VERSION, true);
...
function educational_blocks_admin_theme_style() {
   ...
   wp_enqueue_style('educational-blocks-admin-upsell-style', esc_url(get_template_directory_uri()) . '/assets/css/admin-style.css');
}
 ...
 */
add_action('after_switch_theme', 'educational_blocks_redirect_after_activation');
function educational_blocks_redirect_after_activation() {
    if ( ! get_option( 'educational_blocks_activation_redirect', false ) ) {
        update_option( 'educational_blocks_activation_redirect', true );
        wp_safe_redirect( admin_url( 'themes.php?page=educational-blocks-pro' ) );
        exit;
    }
}

add_action( 'admin_bar_menu', 'educational_blocks_add_upgrade_button', 100 );

function educational_blocks_add_upgrade_button( $educational_blocks_wp_admin_bar ) {

    $educational_blocks_theme_name = wp_get_theme()->get( 'Name' );

    $educational_blocks_args = array(
        'id'    => 'educational_blocks_upgrade',
        'title' => '<span style="color:#fff;font-weight:600;">
            🚀 Upgrade to ' . esc_html( $educational_blocks_theme_name ) . ' Pro - 20% OFF 
            <span style="background:#ff5722;color:#fff;padding:2px 8px;border-radius:3px;margin-left:6px;">
                Buy Now
            </span>
        </span>',
        'href'  => esc_url( EDUCATIONAL_BLOCKS_BUY_PRO ),
        'meta'  => array(
            'class'  => 'educational-blocks-upgrade-btn',
            'title'  => 'Upgrade to Pro',
            'target' => '_blank'
        )
    );

    $educational_blocks_wp_admin_bar->add_node( $educational_blocks_args );
}